    ...
    public void collectChatFrames(BuildProducer<ChatFrameBuildItem> chatFrameProducer,
        ...
        for (AnnotationInstance funqMethod : funqs) {
            ...
            if (Modifier.isAbstract(declaringClass.flags()) || Modifier.isInterface(declaringClass.flags())) {
                throw new RuntimeException(
                        String.format("@ChatFrame is not allowed within abstract classes or interfaces. Method '%s' annotated with '@ChatFrame' is declared within the class '%s'.",
                                methodName, className));
            }
            ...
            if (method.parametersCount() != 0) {
                throw new RuntimeException(
                        String.format("Method '%s' annotated with '@ChatFrame' declared in the class '%s' must not have any parameters.",
                                methodName, className));
            }
            ...
            if (defaultFrame) {
                if (defaultFrameFound) {
                    throw new RuntimeException(
                            String.format("Multiple default chat frames found. Only one @DefaultChatFrame chat frame is allowed per deployment.",
                                    methodName, className));
                }
        ...
        if (!classNames.isEmpty()) {
            ...
            additionalBeanProducer.produce(AdditionalBeanBuildItem.builder().addBeanClasses(classNames).setDefaultScope(DotNames.APPLICATION_SCOPED).setUnremovable().build());
         }
    }

    @BuildStep
    @Record(STATIC_INIT)
    public void registerChatFrames(ChatFrameRecorder recorder, RecorderContext context, List<ChatFrameBuildItem> chatFrameBuildItems) {
        if (chatFrameBuildItems.size() == 1) {
            Log.info("There is only one chat frame so setting default chat frame to " + chatFrameBuildItems.get(0).getFrameName());
            ChatFrameBuildItem chatFrame = chatFrameBuildItems.get(0);
            recorder.registerChatFrame(chatFrame.getFrameName(), context.classProxy(chatFrame.getClassName()), chatFrame.getMethodName(),true);
        } else {
            boolean hasDefaultFrame = false;
            for (ChatFrameBuildItem chatFrame : chatFrameBuildItems) {
                Log.info("Registering chat frame: " + chatFrame.getFrameName() + " is default: " + chatFrame.isDefaultFrame());
                recorder.registerChatFrame(chatFrame.getFrameName(), context.classProxy(chatFrame.getClassName()), chatFrame.getMethodName(), chatFrame.isDefaultFrame());
                if (chatFrame.isDefaultFrame()) {
                    hasDefaultFrame = true;
                }
            }
            if (!hasDefaultFrame) {
                Log.warn("No default chat frame found.  Use @DefaultChatFrame on at least one @ChatFrame method, or you will have to pass chat frame with context");
            }
        }
    }
