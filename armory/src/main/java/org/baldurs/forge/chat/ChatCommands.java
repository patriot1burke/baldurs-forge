...
13	StuddedLeather
    ...
    @Tool("Find all unique icons for a given armor type")
    public String findUniqueIconsForArmorType(String armorType) {
        List<RootTemplate> rootTemplates = library.findRootIconsFrom("ArmorType", armorType);
        String html = "<ul>";
        for (RootTemplate rootTemplate : rootTemplates) {
            html += "<li><img src=\"" + library.icons().get(rootTemplate.resolveIcon()) + "\" /> " + rootTemplate.MapKey + "</li>";
        }
        html += "</ul>";
        context.response().add(new MessageAction(html));
        return "I found " + rootTemplates.size() + " unique root templates for armor type: " + armorType;
    }
